...
const Test = () => {
  ...
  const filteredContainers = [
    ...
    },
    {
      ...
      phone: "+91 8765309568",
      address: "Suthanthira Nagar, Gandhi Nagar",
  ...
  const filteredContainersByName = filteredContainers.filter(
    (container) =>
      ...
      {filteredContainersByName.length > 0 ? (
        filteredContainersByName.map((container, index) => (
          <div className="container" key={index}>
            ...
            <button className="Details" onClick={HandleChange}>
              Detail and Book
            </button>

            {container.mapSrc && (
              <iframe
                src={container.mapSrc}
                className="map"
                title="Location Map"
              ></iframe>
            )}
